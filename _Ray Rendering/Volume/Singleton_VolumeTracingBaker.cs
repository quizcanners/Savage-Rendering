...
namespace QuizCanners.VolumeBakedRendering
{
    ...
    public partial class Singleton_VolumeTracingBaker : Singleton.BehaniourBase, IPEGI, IPEGI_Handles
    {
        ...
        private readonly VolumeTexture_ShaderTransform InShaderTransform = new();

        public class VolumeTexture_ShaderTransform
        {
            ...
            public void SetDynamicGlobal(Transform root)
            {
                DYMANIC_VOLUME_WTL_MATRIX.GlobalValue = root.worldToLocalMatrix;
                DYMANIC_VOLUME_LTW_MATRIX.GlobalValue = root.localToWorldMatrix;
                USE_DYNAMIC_VOLUME.GlobalValue = 1;
            }

            public void SetStaticRoot()
            {
        ...
        private Gate.Frame _waitFrames = new();
        ...
        public bool TryChangeOffset()
        {
            if (volume && _bakedPositionAndSize.ValueIsDefined)
            {
                    ...
                    || _bakedPositionAndSize.CurrentValue != newPosition)
                {
                    ...
                    if (!TracedVolume.VolumeExists || (Vector3.Distance(newPosition.XYZ(), _bakedPositionAndSize.CurrentValue.XYZ()) > volume.SliceWidth * volume.size)) 
                    {
                    ...
                    if (rt)
                    {
                        volume.Texture = _doubleBuffer.RenderFromAndSwapIn(rt, offsetShader, andRelease: true);
                    }

                    cubeMapped.OffsetResults(this);
                    Qc_OffsetRGBA.Enabled = true;
                    _doubleBuffer.Blit(offsetShader, andRelease: true);
        ...
        public void ClearBake(bool eraseResult = true) 
        {
            ...
            if (eraseResult && volume.Texture)
            {
                ...
                TracedVolume.BASE_LIGHT_VOLUME_VISIBILITY.GlobalValue = 0;
            }
        ...
        public void WaitABit() => _waitFrames.TryEnter();
        ...
        public void LateUpdate()
        {
            ...
            if (!_waitFrames.IsFramesPassed(2))
                return;
            ...
            void UpdateDynamicRootIfNeeded() 
            {
                ...
                transform.position = last.transform.position;
                InShaderTransform.SetDynamicGlobal(last.transform);
            }

           // if (!VolumeActive)
           // {
                TracedVolume.BASE_LIGHT_VOLUME_VISIBILITY.GlobalValue = QcLerp.LerpBySpeed(TracedVolume.BASE_LIGHT_VOLUME_VISIBILITY.GlobalValue, 1, speed: 2, unscaledTime: true);
          ...
          //  }
            ...
            switch (PositionUpdateMode) 
            {
                ...
                default:
                    ...
                    InShaderTransform.SetStaticRoot();
            ...
            bool BakeMainVolume()
            {
                ...
                switch (_mainBakingStage) 
                {
                    ...
                    case MainBakingStage.PostEffect:
                        ...
                        RenderTexture rt = volume.GetOrCreate() as RenderTexture;
                        _doubleBuffer.BlitTargetWithPreviousAndSwap(ref rt, postEffectsShader, andRelease: true);
                        volume.Texture = rt;
                ...
                void Smooth()
                {
                    ...
                    _doubleBuffer.BlitTargetWithPreviousAndSwap(ref rt, smoothingShader, andRelease: true);
                    volume.Texture = rt;
                }
            }
        }
