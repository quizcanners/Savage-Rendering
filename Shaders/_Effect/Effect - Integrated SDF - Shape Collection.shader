Shader "QcRendering/Effect/Volumetric SDF Shape Collection"
{
	Properties
	{
		...
		Pass{
			...
			fixed4 frag(v2f i) : SV_TARGET
			{
				...
				float size;
					...
					vDirObjSpace = -normalize(Rotate(viewDir, q));

					wCamPosObjSpace = Rotate(_WorldSpaceCameraPos - pos,q);
				...
				float3 newPos;
#if MESH_POS
				newPos = pos + Rotate(ro, inverseQuaternion);
				...
				normal = normalize(Rotate(float4(SdfNormal(ro, wCamPosObjSpace, size), 0), inverseQuaternion));
