...
Shader "QcRendering/Integrated SDF/Volumetric World Space"
{
	Properties	
	{
	...
	SubShader
	{
		...
		CGINCLUDE
			...
			float SampleSDF(float3 pos, float4 spherePos)
			{
				...
				float gyr = sdGyroid(pos + float3(0, -10, 0) , 15 / scale, 0.1, 1);
		...
		//	shadow *= getShadowAttenuation(newPos);
					...
					precomp.traced = 0;
					precomp.water = 0;
					precomp.smoothsness = _Reflectivity; // madsMap.a;
