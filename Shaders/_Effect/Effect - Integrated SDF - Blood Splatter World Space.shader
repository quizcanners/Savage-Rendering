Shader "QcRendering/Integrated SDF/Volumetric Blood Splatter World Space"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			FragColDepth frag(v2f i)
			{
				...
				float3 bakeReflected = GetBakedAndTracedReflection(volumeSamplePosition, reflectedRay, BLOOD_SPECULAR);//SampleReflection(o.worldPos, viewDir, normal, shadow, hit);

				
				

				float3 refractedRay =  refract(-viewDir, normal, 0.75);
				float3 bakeStraight = GetBakedAndTracedReflection(volumeSamplePosition, refractedRay, BLOOD_SPECULAR);
