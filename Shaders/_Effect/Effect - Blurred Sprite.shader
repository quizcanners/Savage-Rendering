Shader "QcRendering/Effect/Blurred Sprite" 
{
	Properties
	{
		...
		SubShader
		{
			Pass
			{
				...
				float shadow = GetSunShadowsAttenuation(i.worldPos, i.screenPos.z);
					...
					float4 result = float4(col * 4, alpha * 0.25);

					result.rgb = TransparentLightStandard(result, i.worldPos, normal, viewDir, shadow);

					ApplyLayeredFog_Transparent(result, screenUV, distToCamera);
