...
//		[KeywordEnum(MADS, None, Separate)] _AO("AO Source", Float) = 0
	...
	//	[Toggle(_SDF_AMBIENT)] sdfAmbient("SDF Ambient", Float) = 0
		...
		_Reflectivity("Refectivity", Range(0,1)) = 1
...
#				endif
				...
				*/
					...
					float2 uvSmplZY = uvHor.zy * tiling;
					...
					float2 uvSmplXY = uvHor.xy * tiling;
					smoothedPixelsSampling(uvSmplXY, _MainTex_TexelSize);
				...
				//	normal = normalize(normal + horNorm * horBumpVaidity);
					...
					normal = normalize(preNormal.xyz + triplanarNorm * _BumpScale * 2);
