Shader "QcRendering/Geometry/Standard Translucent"
{
	Properties
	{
		...
		_BumpMap("Normal Map", 2D) = "bump" {}
			...
			_BumpScale("Bump Intensity", Range(0,2)) = 1
	...
	//	[KeywordEnum(None, Unity, VertexTraced, PixelTraced)] _SHADOW("Shadow", Float) = 0
		...
		[Toggle(_REFLECTED_PART_ONLY)] traceSkyOnly ("Only darkness beyond", Float) = 0  
	...
	Category
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				#pragma shader_feature_local ___ _REFLECTED_PART_ONLY
...
#	endif
			...
			#if _REFLECTED_PART_ONLY
				ApplyBottomFog(bakeReflected, i.worldPos.xyz, viewDir.y);
				return float4(bakeReflected*(1-showStraight),1);
