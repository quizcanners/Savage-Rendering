		...
		SubShader
		{
			...
			Pass
			{
				...
				FragColDepth frag(v2f i)
				{
					...
					float facingSun = dot(i.normal.xyz, _WorldSpaceLightPos0.xyz);
				...
				//	col = ApplyStandardTransparentLighting(tex);
						...
						col += 	tex2D(_SkinMask, uv).rgb * (1-facingSun) * _SubSurface.rgb * _SubSurface.a *(((0.1 + lookingAtSun) *  lightColor)  + topDownBehind.rgb);		
					#endif
					...
					output.col = //float4(normal.xyz, result.a)
						result
						;
					return output;
				}
