#ifndef QC_PRIM_CUBESMP
#define QC_PRIM_CUBESMP
...
float3 SampleVolume_CubeMap_Internal(float3 pos, float3 normal)
{
	float outOfBounds;

	float upperFraction;
	float4 uvsBake = WorldPosToVolumeUV(pos, _RayMarchingVolumeVOLUME_POSITION_N_SIZE, _RayMarchingVolumeVOLUME_H_SLICES, upperFraction, outOfBounds);

	float3 bake = SampleVolume_Internal(_RayMarchingVolume, uvsBake, upperFraction).rgb; // * 2; // To compensate brightness
	...
	float outOfBoundsCube;
	float4 uvs = WorldPosTo_CubeLight_VolumeUV(pos, upperFraction, outOfBoundsCube);
	...
	float showCube = (1-outOfBoundsCube) * _RT_CubeMap_FadeIn;

	float3 cubeBake = 0;
	cubeBake += absDir.x * lerp(bake, toAddX, showCube); //smoothstep(0, 5, toAddX.a * _RT_CubeMap_FadeIn));
	cubeBake += absDir.y * lerp(bake, toAddY, showCube); //smoothstep(0, 5, toAddY.a * _RT_CubeMap_FadeIn));
	cubeBake += absDir.z * lerp(bake, toAddZ, showCube); //smoothstep(0, 5, toAddZ.a * _RT_CubeMap_FadeIn));

	
	//bake = lerp(bake, cubeBake, saturate(cubeBake.a*0.01)); 

	return cubeBake * (1-outOfBounds) * qc_VolumeAlpha;
}
...
float3 SampleVolume_CubeMap(float3 pos, float3 normal)
{
		...
		float outOfBoundsCube;
		float upperFractionCubemap;
		float4 uvs = WorldPosTo_CubeLight_VolumeUV(pos, upperFractionCubemap, outOfBoundsCube);

		float showCube = (1-outOfBoundsCube) * _RT_CubeMap_FadeIn;

		float3 bake = SampleVolume_Internal(_RayMarchingVolume, uvsBake, upperFraction).rgb; // * 2; // To compensate brightness

		float flatBakeAlpha = qc_VolumeAlpha * (1-outOfBounds);
		...
		{	
			...
			float showDirectBake = (1-qc_VolumeAlpha)*(1-directOOb);

			flatBakeAlpha = lerp(flatBakeAlpha, 1, showDirectBake);

			bake = lerp(bake, directSample, showDirectBake);
		} 

		float showBake = lerp(showCube,flatBakeAlpha,flatBakeAlpha);
		...
		float4 toAddX = SampleCubemap_Internal(uvs, upperFractionCubemap, isLeft);
		float4 toAddY = SampleCubemap_Internal(uvs, upperFractionCubemap, 2 + isDown);
		float4 toAddZ = SampleCubemap_Internal(uvs, upperFractionCubemap, 4 + isBack);

		float3 cubeBake = 0;
		cubeBake += absDir.x * lerp(bake, toAddX.rgb, showCube); // smoothstep(0, 5, toAddX.a * _RT_CubeMap_FadeIn));
		cubeBake += absDir.y * lerp(bake, toAddY.rgb, showCube); // smoothstep(0, 5, toAddY.a * _RT_CubeMap_FadeIn));
		cubeBake += absDir.z * lerp(bake, toAddZ.rgb, showCube); // smoothstep(0, 5, toAddZ.a * _RT_CubeMap_FadeIn));
		...
		*/
		 ...
		 // lerp(bake, cubeBake, _RT_CubeMap_FadeIn); 
...
//#endif
	...
	return lerp(avgAmb,cubeBake, showBake); //lerp(1, showAmbient, flatBakeAlpha));
}
