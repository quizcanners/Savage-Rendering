Shader "QcRendering/Baker/RayBakingOfLight"
{
	Properties
	{
		...
		Pass
		{
			...
			//	float3 worldPos = volumeUVtoWorld(o.texcoord.xy);
				...
				float blackPixel;
					...
					blackPixel = (1-saturate(dot(sdf0.xyz, _RT_CubeMap_Direction.xyz))) 
					* 
					smoothstep(VOL_SIZE*3, 0, sdf0.a) * (1-outOfBounds);
				#else
					blackPixel = smoothstep(0, -VOL_SIZE*0.5, sdf0.a) * (1-outOfBounds);
				#endif
			...
			//	float sdfOffsetAmount = smoothstep(VOL_SIZE, 0, nrmDist.w);
				...
				float closeToWall = saturate(1-sdf0.a * 0.5 / VOL_SIZE);

				float dotToNorm = dot(sdf0.xyz, rayDirection);

				float flipRay = step(0, -dotToNorm) * closeToWall * outOfBounds;
