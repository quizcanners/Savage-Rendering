...
#include "Assets\Qc_Rendering\Terrain Stamps\Shaders/Qc_TerrainCommon.cginc"
...
float4 SampleVolumetricLight(float3 pos, float dist, float facingSun)
{
	float4 sampled = 0;
		...
		float outOfBounds;
		float3 baked = SampleVolume(pos, outOfBounds).rgb;
	...
	//	sampled += smokeAndFire;
		...
		float nearSurface = saturate((1 + level * 2) - scene.a) * (1 - outsideVolume);
		...
		//showRain =   lerp(1, 0.5 + nearSurface * 10 - noise.y * (1 + level), showRain);

		#if !_qc_IGNORE_SKY
			showRain *= atten; // In Shadow is likely to be under the roof
		#endif

		visibilityMode *= showRain;
					
	#endif

	return sampled * visibilityMode;
}
