...
float3 Sdf_VolumeUVtoWorld(float2 uv) 
{
	if (Qc_SDF_USE_DYNAMIC_RTX_VOLUME > 0)
	{
		float4 zeroPos = float4(0,0,0, Qc_SDF_VOLUME_POSITION_N_SIZE.w);
		float3 localPos = volumeUVtoWorld(uv, zeroPos, Qc_SDF_VOLUME_H_SLICES);

		return mul(Qc_SDF_LocalToWorld, float4(localPos,1)).xyz;
	}

	return volumeUVtoWorld(uv, Qc_SDF_VOLUME_POSITION_N_SIZE, Qc_SDF_VOLUME_H_SLICES);
}
...
{
	float edges = Qc_SDF_VOLUME_POSITION_N_SIZE.w * 0.5 * smoothness;
