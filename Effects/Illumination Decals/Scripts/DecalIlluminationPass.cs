...
namespace QuizCanners.SpecialEffects
{
    ...
    public class DecalIlluminationPass : IPEGI
    {
        ...
        private void PopulateBuffer(List<C_ShadowDecalTarget> targets, CommandBuffer buffer)
        {
            ...
            {
                var targetMesh = target.GetMesh(mesh);
                if (!targetMesh)
                {
                    continue;
                }
                ...
                buffer.DrawMesh(targetMesh, Matrix4x4.TRS(tf.position + scale * 1.5f * sunDirection, rot, new Vector3(scale, scale, scale * 4)), GetMaterial(target), 0, 0);
        ...
        private void PopulateBuffer(List<C_AODecalTarget> targets, CommandBuffer buffer) 
        {
            foreach (C_AODecalTarget target in targets)
            {
                ...
                buffer.DrawMesh(targetMesh, Matrix4x4.TRS(tf.position, tf.rotation, tf.lossyScale), GetMaterial(target), 0, 0);
