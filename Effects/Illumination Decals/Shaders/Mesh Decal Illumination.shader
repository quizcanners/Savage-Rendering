Shader "Quiz cAnners/Illumination/Ao Decal" 
{
	Properties
	{
		//[HDR]_Color("Color (R - AO B - shadow)", Color) = (1,1,1,1)
			...
			Pass
			{
				...
				float4 frag(v2fMarchBatchableTransparent i) : COLOR
				{
					...
					float dist;
						...
						dist = length(pow(abs(localOffset), 2 + boxing * 9)) * (1 + pow(boxing*10, 1+ boxing * 5)); 
					...
					float distFade = 1/(1+pow(dist * 20, 2));
					...
					col *= distFade * maxExtend * maxExtendSharp;
