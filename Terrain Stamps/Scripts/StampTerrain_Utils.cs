...
namespace QuizCanners.StampTerrain
{
    ...
    {
        public static List<C_TerrainAllign_Base> s_allignToTerrainTargets = new();

       

       
      
        public static class EmbankmentManagement 
        {
            
            public static List<C_BlanketShaderColliderAlligner> s_colliderAlligners = new();

            private static int _inspectedCollider = 0;
            private static readonly Gate.Integer _terrainVersion = new();

            public static void ManagedUpdate() 
            {
                if (_terrainVersion.TryChange(AllignToTerrainVersion))
                    _inspectedCollider = 0;

                if (_inspectedCollider >= s_colliderAlligners.Count)
                    return;

                s_colliderAlligners[_inspectedCollider].AllignCollider();

                _inspectedCollider++;
            }

            public static void Add(C_BlanketShaderColliderAlligner collider) => s_colliderAlligners.Add(collider);

            public static void Remove(C_BlanketShaderColliderAlligner collider) => s_colliderAlligners.Remove(collider);

        }

        public static int AllignToTerrainVersion;

        public static void AllignToTerrain(Transform transform, bool allignRotation, float yOffset = 0)
        {
            var pos = transform.position;

            if (!Singleton.TryGet<TerrainManagerBase>(out var sng)) 
            {
                Debug.LogError("No Terrain Manager");
                return;
            }

            if (allignRotation)
            {
                var normal = sng.GetNormal(pos);
                transform.rotation = Quaternion.FromToRotation(transform.up, normal) * transform.rotation;
            }

            pos.y = sng.GetTerrainHeight(pos) + yOffset;
        ...
        public static class Layers
        {
            ...
            public static void ManagedUpdate() 
            {
                ...
                SETTINGS.GlobalValue = new Vector4(currentlySet.Settings.Sharpness, currentlySet.Settings.LayerHeightImportance, 0, 0);
            }
