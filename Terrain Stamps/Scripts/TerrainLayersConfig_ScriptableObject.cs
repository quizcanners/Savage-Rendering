...
namespace QuizCanners.StampTerrain
{
    ...
    public class TerrainLayersConfig_ScriptableObject : ScriptableObject, IPEGI
    {
        ...
        [Serializable]
        public class Configurations : IPEGI
        {
            public float Sharpness = 0.1f;
            public float LayerHeightImportance = 0.5f;

            public void Inspect()
            {
                "Sharpness".PegiLabel(50).Edit(ref Sharpness, 0.001f, 0.2f).Nl();
                "Layer Height Importance".PegiLabel().Edit_01(ref LayerHeightImportance).Nl();
            }
        }
    }
}
